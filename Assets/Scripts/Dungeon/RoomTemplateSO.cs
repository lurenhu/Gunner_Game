...
public class RoomTemplateSO : ScriptableObject
{
    ...
    public List<SpawnableObjectByLevel<EnemyDetailSO>> enemyByLevelList;
    ...
    private void OnValidate()
    {
        ...
        HelperUtility.ValidateCheckNullValue(this, nameof(prefab), prefab);
        HelperUtility.ValidateCheckNullValue(this, nameof(roomNodeType), roomNodeType);

        HelperUtility.ValidateCheckEnumerableValue(this, nameof(doorwayList), doorwayList);

        if (enemyByLevelList.Count > 0 || roomEnemySpawnParameterList.Count > 0)
        {
            HelperUtility.ValidateCheckEnumerableValue(this, nameof(enemyByLevelList), enemyByLevelList);
            HelperUtility.ValidateCheckEnumerableValue(this, nameof(roomEnemySpawnParameterList), roomEnemySpawnParameterList);

            foreach (RoomEnemySpawnParameters roomEnemySpawnParameters in roomEnemySpawnParameterList)
            {
                HelperUtility.ValidateCheckNullValue(this, nameof(roomEnemySpawnParameters.dungeonLevel), roomEnemySpawnParameters.dungeonLevel);
                HelperUtility.ValidateChackPositveRange(this,nameof(roomEnemySpawnParameters.minTotalEnemiesToSpawn), 
                ...
                HelperUtility.ValidateChackPositveRange(this, nameof(roomEnemySpawnParameters.minSpawnInterval),
                ...
                HelperUtility.ValidateChackPositveRange(this, nameof(roomEnemySpawnParameters.minConcurrentEnemies),
                ...
                foreach (SpawnableObjectByLevel<EnemyDetailSO> dungeonObjectsByLevel in enemyByLevelList)
                {
                    ...
                    HelperUtility.ValidateCheckNullValue(this, nameof(dungeonObjectsByLevel.dungeonLevel), dungeonObjectsByLevel.dungeonLevel);

                    foreach (SpawnableObjectRatio<EnemyDetailSO> dungeonObjectRatio in dungeonObjectsByLevel.spawnableObjectRatioList)
                    {
                        HelperUtility.ValidateCheckNullValue(this, nameof(dungeonObjectRatio.dungeonObject), dungeonObjectRatio.dungeonObject);

                        HelperUtility.ValidateCheckPostiveValue(this, nameof(dungeonObjectRatio.ratio), dungeonObjectRatio.ratio, false);
                    }
                }

                if (isEnemyTypesListForDungeonLevel == false && roomEnemySpawnParameters.dungeonLevel != null)
                {
                    Debug.Log("No enemy type specify in for dungeon level " + roomEnemySpawnParameters.dungeonLevel.levelName + " in gameobject "
                        + this.name.ToString());
                }
            }
        }
        ...
        HelperUtility.ValidateCheckEnumerableValue(this, nameof(spawnPositionArray), spawnPositionArray);
    }
